...
def solve_heat_equation(
    ...
    for i in range(0, cfg.nt-1):
        ...
        # Right-hand side
        b = _build_rhs(cfg, T_curr, X, Y, cx, cy, dx, dy, dt, t_next)
...
def _build_rhs(cfg: Config, T_curr, X, Y, cx, cy, dx, dy, dt, t_next):
    """Build right-hand side for implicit system."""
    rhs = T_curr.copy()

    # Heat source
    q = np.array(cfg.heat_source_am(X, Y, t_next, cx, cy))
    rhs += dt * q
