...
def sample_interior(key, cfg: Config):
    ...
    sampled = _sample_qmc(
        cfg.num_collocation,
        mins=[cfg.x_min, cfg.y_min, cfg.t_min, cfg.x_min, cfg.y_min], # cx, cy limits
        maxs=[cfg.x_max, cfg.y_max, cfg.t_max, cfg.x_max, cfg.y_max], # cx, cy limits
        seed=seed,
    )
...
def sample_ic(key, cfg: Config):
    ...
    pts = _sample_qmc(
        cfg.num_ic,
        mins=[cfg.x_min, cfg.y_min, cfg.x_min, cfg.y_min],  # x, y, cx, cy
        maxs=[cfg.x_max, cfg.y_max, cfg.x_max, cfg.y_max],
        seed=seed,
    )

    # Unpack results from QMC
    x, y = pts[:, 0:1], pts[:, 1:2]
    cx, cy = pts[:, 2:3], pts[:, 3:4]
    
    # Create t=0 column
    t0 = jnp.zeros((cfg.num_ic, 1))

    # Concatenate to [x, y, 0, cx, cy]
    result = jnp.concatenate([x, y, t0, cx, cy], axis=1)
...
def sample_bc(key, cfg: Config):
    ...
    # Use uniform sampling for source locations
    key, k1, k2 = jax.random.split(key, 3)
    cx = jax.random.uniform(k1, (n,), minval=cfg.x_min, maxval=cfg.x_max)
    cy = jax.random.uniform(k2, (n,), minval=cfg.y_min, maxval=cfg.y_max)

    # Stack all together: [x, y, t, nx, ny, cx, cy]
    return jnp.stack([x, y, t, nx, ny, cx, cy], axis=1), key
